			...
			if (reservationNo.equals(cs.reservations.get(i).getReservationNo())) {
				...
				for (int j = 0; j < cs.theaters.size(); j++) {
					if (cs.theaters.get(j).getMovie().equals(m)) {
						System.out
								.println((j + 1) + "번 상영관, 좌석은 [" + r.getSeat()[0] + " - " + r.getSeat()[1] + "] 입니다.");
						break;
					}
				}
				match = true;
		...
		if (isLoggedIn) {
			if (password.equals(cs.userLoggedIn.getUserPassword())) {
				cs.userLoggedIn.setUserPoint(
						cs.userLoggedIn.getUserPoint() - (int) (cs.reservations.get(i).getTheater().getPrice() * 0.1));
...
//			}
			...
			System.out.println(
					"고객님의 예약번호는 " + cs.reservations.get(cs.reservations.size() - 1).getReservationNo() + "입니다.");
